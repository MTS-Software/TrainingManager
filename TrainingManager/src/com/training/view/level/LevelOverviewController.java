...
public class LevelOverviewController {
	...
	public void setData() {
		...
		ObservableList<Level> levelenFX = FXCollections
				.observableArrayList(Service.getInstance().getLevelService().findAll());
		table.setItems(levelenFX);

	}

	private void showDetails(Level level) {

		levelDataController.setData(level);
		levelDataController.setEditable(false);
	}
	...
	private void handleDelete() {
		...
		if (selectedIndex >= 0) {
			...
			if (alert.isOKButton()) {
				...
				Service.getInstance().getLevelService().delete(data);
	...
	private void handleNew() {
		...
		if (okClicked) {
			table.setItems(FXCollections.observableArrayList(Service.getInstance().getLevelService().findAll()));
			table.refresh();
			showDetails(null);
		}
	}
	...
	private boolean showEditDialog(Level data) {
		try {

			FXMLLoader loader = new FXMLLoader();
			loader.setLocation(Main.class.getResource("view/level/LevelEdit.fxml"));
			...
			dialogStage.setTitle("Level: Bearbeiten");

			Scene scene = new Scene(page);
			scene.getStylesheets().add(Constants.STYLESHEET);
			dialogStage.setScene(scene);

			LevelEditController controller = loader.getController();
