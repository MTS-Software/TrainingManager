...
public class KategorieOverviewController {
	...
	@FXML
	private TableColumn<Kategorie, String> nameColumn;
	...
	public void setData() {
		...
		ObservableList<Kategorie> abteilungenFX = FXCollections
				.observableArrayList(Service.getInstance().getKategorieService().findAll());
		table.setItems(abteilungenFX);

	}

	private void showDetails(Kategorie abteilung) {

		kategorieDataController.setData(abteilung);
		kategorieDataController.setEditable(false);
	}
	...
	private void handleDelete() {
		...
		if (selectedIndex >= 0) {
			...
			if (alert.isOKButton()) {
				...
				Service.getInstance().getKategorieService().delete(data);
	...
	private void handleNew() {
		...
		Kategorie data = new Kategorie();

		boolean okClicked = showEditDialog(data);
		if (okClicked) {
			table.setItems(FXCollections.observableArrayList(Service.getInstance().getKategorieService().findAll()));
			table.refresh();
			showDetails(null);
		}
	}
	...
	private boolean showEditDialog(Kategorie data) {
		try {

			FXMLLoader loader = new FXMLLoader();
			loader.setLocation(Main.class.getResource("view/kategorie/KategorieEdit.fxml"));
			...
			KategorieEditController controller = loader.getController();
